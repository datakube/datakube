...
type BackupTargetLister interface {
	...
	List(selector labels.Selector) (ret []*v1.BackupTarget, err error)
...
// List lists all BackupTargets in the indexer.
func (s *backupTargetLister) List(selector labels.Selector) (ret []*v1.BackupTarget, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.BackupTarget))
	})
	return ret, err
}
...
type BackupTargetNamespaceLister interface {
	...
	Get(name string) (*v1.BackupTarget, error)
...
// List lists all BackupTargets in the indexer for a given namespace.
func (s backupTargetNamespaceLister) List(selector labels.Selector) (ret []*v1.BackupTarget, err error) {
...
// Get retrieves the BackupTarget from the indexer for a given namespace and name.
func (s backupTargetNamespaceLister) Get(name string) (*v1.BackupTarget, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("backuptarget"), name)
	}
	return obj.(*v1.BackupTarget), nil
}
